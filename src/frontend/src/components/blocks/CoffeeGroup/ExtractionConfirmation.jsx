...
const ExtractionConfirmation = ({ profile, onConfirm, onCancel, t, isPending = false }) => {
  return (
    <div
      className={cn(
        'flex flex-col h-full bg-accent-red/5 rounded-[1.5rem] p-[1.5rem] border border-accent-red/10 animate-in fade-in duration-300',
    ...
    >
      <div className="flex-1 flex flex-col items-center justify-center text-center">
        <span className="text-text-muted text-[0.75rem] font-black uppercase tracking-widest mb-[0.5rem]">
          {isPending ? t('starting') || 'Запуск...' : t('ready_to_start') || 'Готов к запуску'}
        </span>
      ...
      <div className="flex gap-[1rem]">
        <button
          onClick={onCancel}
          disabled={isPending}
          className="flex-1 h-[3.5rem] rounded-[1rem] bg-surface-light border border-white/5 text-text-secondary font-black uppercase tracking-wider active:scale-95 transition-transform disabled:opacity-50"
        >
          {t('cancel') || 'ОТМЕНА'}
        </button>
        <IconButton
          ...
          onClick={onConfirm}
          className={cn(
            'h-[3.5rem] w-[3.5rem] rounded-[1rem] shadow-glow-red',
